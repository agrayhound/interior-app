  ...
  const { error } = await supabase.from("suppliers").upsert(
    {
      id:      SUPPLIER_ID,
      name:    "Centanni Tile",
      website: "https://www.centannitile.com",
      city:    "Vancouver",
      active:  true,
    },
    { onConflict: "id" }
...
async function main() {
  const raw = JSON.parse(readFileSync("centanni_products.json", "utf8"));
  console.log(`Loaded ${raw.length} products from centanni_products.json\n`);
