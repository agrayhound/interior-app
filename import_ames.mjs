/**
 * import_ames.js — Import Ames Tile products into Supabase.
 * Run: node import_ames.js
 */
  ...
  const { error } = await supabase.from("suppliers").upsert(
    {
      id:      SUPPLIER_ID,
      name:    "Ames Tile",
      website: "https://www.amestile.com",
      city:    "Vancouver",
      active:  true,
    },
    { onConflict: "id" }
...
async function main() {
  const raw = JSON.parse(readFileSync("ames_products.json", "utf8"));
  console.log(`Loaded ${raw.length} products from ames_products.json\n`);
  ...
  // ── Fetch back product IDs (paginated — Supabase max 1000/request) ─────────
  const PAGE = 1000;
