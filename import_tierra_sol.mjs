  ...
  const { error } = await supabase.from("suppliers").upsert(
    {
      id:      SUPPLIER_ID,
      name:    "Tierra Sol",
      website: "https://www.tierrasol.ca",
      city:    "Vancouver",
      active:  true,
    },
    { onConflict: "id" }
...
async function main() {
  const raw = JSON.parse(readFileSync("tierra_sol_products.json", "utf8"));
  console.log(`Loaded ${raw.length} products from tierra_sol_products.json\n`);
