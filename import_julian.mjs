  ...
  const { error } = await supabase.from("suppliers").upsert(
    {
      id:      SUPPLIER_ID,
      name:    "Julian Tile",
      website: "https://www.juliantile.com",
      city:    "Vancouver",
      active:  true,
    },
    { onConflict: "id" }
...
async function main() {
  const raw = JSON.parse(readFileSync("julian_products.json", "utf8"));
  console.log(`Loaded ${raw.length} products from julian_products.json\n`);
