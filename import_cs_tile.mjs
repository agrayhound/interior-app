  ...
  const { error } = await supabase.from("suppliers").upsert(
    {
      id:      SUPPLIER_ID,
      name:    "C&S Tile",
      website: "https://cstile.ceramstone.com",
      city:    "Vancouver",
      active:  true,
    },
    { onConflict: "id" }
...
async function main() {
  const raw = JSON.parse(readFileSync("cs_tile_products.json", "utf8"));
  console.log(`Loaded ${raw.length} products from cs_tile_products.json\n`);
